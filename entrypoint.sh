...
if [[ $UPDATE_GEOIP == "yes" ]]; then
    ...
    if [ $(( $(date +%s) - $(cat /usr/local/var/ntop/geoip-age) )) -gt 604800 ]; then
        ...
        if curl -L https://mailfud.org/geoip-legacy/GeoIPCity.dat.gz -O /tmp/GeoIPCity.dat.gz ; then
            zcat /tmp/GeoIPCity.dat.gz > /usr/local/var/ntop/GeoLiteCity.dat
            rm /tmp/GeoIPCity.dat.gz
        fi
        if curl -L https://mailfud.org/geoip-legacy/GeoIPASNum.dat.gz -O /tmp/GeoIPASNum.dat.gz ; then
